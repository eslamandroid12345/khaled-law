...
@endphp
@section('title', __('dashboard.uses'))
@section('content')
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">@lang('dashboard.uses')</h3>
                            <div class="card-tools">
                                @permission('uses-create')
                                <a href="{{ route('uses.create') }}"
                                   class="btn  btn-dark">@lang('dashboard.Create')</a>
                                @endpermission
                            </div>
                        </div>
                        <div class="card-body">
                            <form action="{{route('uses.index')}}">
                                <div class="row">
                                    ...
                                    <div class="form-group col-3">
                                        ...
                                        <a href="{{route('uses.index')}}" style="color: #fff" class="btn btn-dark waves-effect waves-light"><i class="fa fa-spinner"></i> </a>
                                    </div>
                                </div>
                            </form>
                            ...
                            <table class="table table-bordered">
                                ...
                                @forelse($uses as $use)
                                    <tr>
                                        ...
                                        <td>{{ $use->t('title') }}</td>
                                        <td><img src="{{ !is_null($use->image) ? url($use->image?->image_url) : '' }}" style="width: 100px;" /></td>
                                      <td>
                                            <div class="operations-btns" style="">
                                                @permission('uses-update')
                                                <a href="{{ route('uses.edit', $use->id) }}"
                                                   class="btn  btn-dark">@lang('dashboard.Edit')</a>
                                                @endpermission
{{--                                                <a href="{{ route('services.show', $service->id) }}"--}}
{{--                                                   class="btn  btn-dark">@lang('dashboard.Show')</a>--}}
                                                @permission('uses-delete')
                                                        ...
                                                        <div class="modal-dialog">
                                                            <div class="modal-content float-left">
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <form
                                                                        action="{{ route('uses.destroy', $use->id) }}"
                                                                        method="post">
                                                                        @csrf
                                                                        {{ method_field('delete') }}
                                                                        <button type="submit"
                        ...
                        <div class="card-footer">
                            {{ $uses->appends(['search' => request()->search])->links() }}
                        </div>
                    </div>
                    <!-- /.card -->
                </div>
